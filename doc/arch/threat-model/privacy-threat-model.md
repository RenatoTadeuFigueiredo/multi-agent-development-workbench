...
| PT-17 | data-disclosure | MCP tool arguments, results, environment values, header secrets, or frames leak through durable history, public protocol, locks, or logs | tool I/O, secret handles, MCP frames, executable paths | gateway persists only redacted lifecycle metadata; secret handles stay opaque; raw arguments/results/frames/paths are forbidden on every public surface |
| PT-18 | non-compliance | provider-local MCP registration or unpinned MCP package/endpoint drift grants tools outside the central allowlist | shared tools, repository data, external services | daemon-owned gateway pins version and sha256, intersects role/workflow/user policy, requires approval for protected effect classes, and keeps Features 004–006 provider MCP maps empty |
