...
- [x] T004 Create `Cargo.toml`, `rust-toolchain.toml`, `.cargo/config.toml`, and
  the seven crates declared in `plan.md`, pinning Rust 1.95.0 and every direct
  dependency.
- [x] T005 [P] Configure workspace formatting, warnings-denied Clippy,
  unsafe-code policy, release profiles, license metadata, and baseline
  `build`, `fmt`, `lint`, and `test` targets in `Makefile`.
- [x] T006 [P] Generate checked-in fixtures from AsyncAPI, the five JSON
...
- [x] T007 Implement UUIDv7 identifiers, sequences, cursors, hashes, validated
  value objects, and the closed redacted error taxonomy in
  `crates/workbench-core/src/{identity,value,error}.rs`. [FR-005, FR-016]
- [x] T008 Implement domain events, causation metadata, effect classes,
...
- [x] T009 Implement legal session states, deterministic folding, serialized
...
- [x] T010 [P] Implement attempt lifecycle, five-second cancellation,
...
- [x] T011 [P] Implement the side-effect-free ordered routing chain and
  single-executor routing plan in `crates/workbench-core/src/routing.rs`.
  [FR-006–FR-008; SC-4, SC-5]
- [x] T012 [P] Implement monotonic policy intersection, protected-action
...
- [x] T013 Implement orchestration services that persist input, route,
...
- [x] T014 [P] Add unit and property tests for folding, sequences, controls,
...
- [x] T015 Implement schema-matching configuration models and exact safe
  defaults for the fake-provider profile in
  `crates/workbench-config/src/model.rs` and `fixtures/builtins.yaml`.
- [x] T016 Implement safe macOS/Linux source discovery and independently
...
- [x] T017 [P] Implement cross-reference validation, aliases, central MCP/tool
...
- [x] T022 Create the forward-only initial SQLite migration and safe open,
...
- [x] T023 [P] Implement XChaCha20-Poly1305 with CSPRNG nonces, authenticated
  schema/session/event metadata, and zeroized buffers in
  `crates/workbench-storage/src/crypto.rs`. [FR-019; SC-18]
- [x] T024 Define `KeyStore` and `KeyManager`, fixed record attributes,
...
- [x] T025 [P] Implement non-synchronizable macOS Keychain records and
  fail-closed error mapping in `key_store/macos.rs`. [FR-019, FR-022; SC-19]
- [x] T026 [P] Implement Linux Secret Service login-collection records and
  fail-closed error mapping in `key_store/linux.rs`. [FR-019, FR-022; SC-19]
- [x] T027 Implement encrypted transactional append, monotonic sequence
...
- [x] T028 Implement atomic session creation across key store and SQLite with a
  durable creation journal, proven failure compensation, base/session lock
  storage, and location-bound isolation from unproven cross-database key
  envelopes in `crates/workbench-storage/src/{sqlite,key_store}.rs`.
  [FR-004, FR-019]
- [x] T029 Implement startup recovery of started attempts without definite
  terminal facts and preserve blocked `outcome_unknown` state in
  `crates/workbench-storage/src/recovery.rs`. [FR-013, FR-020; SC-15]
- [x] T030 Implement terminal-only retention, the resumable deletion journal,
...
- [x] T031 Implement streaming age v1 export with canonical version-1 NDJSON
  manifest, encrypted staging and resumable journal, atomic no-overwrite
  publication, absolute owner-only output, no symlink/plaintext temporary
  file, and no key material in
  `crates/workbench-storage/src/export.rs`. [FR-023; SC-21]
- [x] T032 Add cryptographic vectors, tampered-AAD, nonce, encrypted WAL,
...
- [x] T033 [P] Add a common key-store conformance suite plus target-specific
...
- [x] T034 Implement method-specific result DTOs and generated wire mappings
...
- [x] T035 [P] Implement strict NDJSON parsing, duplicate-key/UTF-8/trailing
...
- [x] T036 [P] Implement the 1,024-event/8-MiB bounded subscriber queue,
...
- [x] T037 Implement safe runtime paths, owner-only permissions, symlink
...
- [x] T038 Implement same-user Unix sockets, peer credential verification
...
- [x] T039 Compose configuration, lock verification, key store, migrations,
  recovery, registry, journal-proven orphan cleanup, and graceful shutdown in
  `crates/workbench-daemon/src/{startup,runtime}.rs`.
- [x] T040 Implement `status.get`, atomic `session.create`, `session.get`,
...
- [x] T041 Implement prompt ordering, routing, capability preflight,
...
- [x] T042 [P] Implement pause, resume, redirect, cancellation deadline, and
  multi-client control fan-out in
  `crates/workbench-daemon/src/services/controls.rs`.
  [FR-012, FR-013; SC-8, SC-9, SC-14, SC-15]
- [x] T043 [P] Implement approval and reconciliation services that persist the
...
- [x] T044 [P] Add structured redacted tracing, bounded-label metrics,
...
- [x] T045 Implement fake provider, coordinator, clock, key store, tool,
  telemetry sink, fixtures, and fail-fast network denial in
  `crates/workbench-testkit/src/`. [FR-017; SC-23]
- [x] T046 [P] Add reusable provider and client contract suites in
  `crates/workbench-testkit/src/contracts/{provider,client}.rs` covering
  provider discovery, authentication, resume, stream, cancel, tool events and
  normalized failures plus live local-protocol methods, event replay, and
  method result schemas. [FR-009, FR-011, FR-016]
- [x] T047 [P] Implement the documented CLI tree, protocol client, request
  IDs, cursor handling, human/JSON output, exit codes, and signal ownership in
  `crates/workbench-cli/src/{args,client,commands,output}.rs` and `src/main.rs`.
- [x] T048 Wire daemon startup, one-shot shared startup-configuration commands,
  sessions, prompts, controls, approvals, reconciliation, export, deletion,
  and status in `crates/workbench-cli/src/main.rs`.
- [x] T049 [P] Add CLI command, JSON schema, stderr, exit-code, stdin, UUIDv7,
  and SIGINT contract tests under `crates/workbench-cli/tests/`.

## Phase 7 — Acceptance, Platforms, and Release Evidence

- [x] T050 Bind SC-1–SC-7 for encrypted execution, configuration, routing, and
  capability behavior in `crates/workbench-testkit/tests/feature_001.rs`.
- [x] T051 Bind SC-8–SC-10 for shared controls, append-only redirect, and
  cursor replay/deduplication using two independent clients.
- [x] T052 Bind SC-11–SC-13 for negotiation, unauthorized peers, frame limits,
  and slow-client isolation on macOS and Linux.
- [x] T053 Bind SC-14–SC-17 for cancellation, uncertain outcomes, monotonic
  policy, and approval ordering with the fake clock and tool.
- [x] T054 Bind SC-18–SC-21 for encrypted persistence, key-store failure,
  retention, age export, deletion crash recovery, and cryptographic erasure.
- [x] T055 Bind SC-22–SC-23 for durable request replay and the zero-network,
  zero-paid-quota default suite; fail when any feature scenario is unbound.
- [x] T056 Add SLO harnesses for 1,000 routes, healthy fan-out, 10,000-event
  replay, controls, and cancellation; update the active GitHub CI for explicit
  Linux/macOS gates and keep the non-gating GitLab portability mirror aligned
  for contract drift and security/license/SBOM checks.
- [x] T057 Write the executable feature quickstart and update `README.md`,
...
- [x] T058 Run formatting, Clippy, workspace tests, contract suites, platform
