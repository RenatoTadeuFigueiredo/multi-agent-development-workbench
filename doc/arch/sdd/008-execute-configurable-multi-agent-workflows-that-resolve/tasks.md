...
- [x] T001 Extend workflow configuration validation (and optional `fallbacks`
...
- [x] T002 Add domain `WorkflowRun` state machine in `workbench-core` with
...
- [x] T003 Emit durable workflow transition facts (bounded ids/phase/iteration)
  through the session event path without storing raw prompts or tool payloads.
- [x] T004 Compose workflow start and step dispatch in `workbench-daemon`
  using existing orchestrator attempts, routing plans (`SelectedRule::Workflow`),
  and provider registry resolution from step roles.
- [x] T005 Implement sequential advancement and bounded `on_findings` loops
  with `max_iterations` ceiling and `awaiting_human` when exhausted.
- [x] T006 Wire pause, resume, cancel, and redirect controls to the active
  workflow run with durable phase updates.
- [x] T007 Implement recovery: rebuild active run from events after restart;
  preserve `outcome_unknown` for interrupted attempts.
- [x] T008 Integrate step tool allowlists with Feature 007 gateway policy
  (deny-before-transport proof in acceptance).
- [x] T009 Implement Feature 008 offline acceptance harness in
...
- [x] T010 Update ADR 0009 to accepted, operations notes if needed, README
  capability line, and `docs/project/STATUS.md`.
- [x] T011 Run `speckit validate`, `make check`, and Feature 008 acceptance;
