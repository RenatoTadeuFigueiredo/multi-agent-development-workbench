...
- [ ] T001 Extend configuration and lock contracts for MCP launch identity:
...
- [ ] T002 Scaffold `workbench-mcp` in the workspace with `forbid(unsafe_code)`,
...
- [ ] T003 Implement pin verification and fail-closed availability: compare
...
- [ ] T004 Implement supervised stdio MCP lifecycle: direct argv launch without
...
- [ ] T005 Implement HTTP MCP client: pinned endpoint identity, TLS for
...
- [ ] T006 Wire role and workflow allowlists into policy resolution so
  ...
  user-global denies.
- [ ] T007 Integrate approval gating with the existing protocol: for mandatory
...
- [ ] T008 Implement gateway invoke path and attempt semantics: stable attempt
...
- [ ] T009 Compose the gateway in `workbench-daemon` startup, health, and
  ...
  the gateway.
- [ ] T010 Add committed offline fakes (`fake_mcp_stdio` / `fake_mcp_http` or
...
- [ ] T011 Implement Feature 007 acceptance harness in `workbench-testkit`:
  ...
  defaults.
- [ ] T012 Prove secret containment: unique markers in arguments, results,
...
- [ ] T013 Update threat model, operations/runbook notes, README capability
