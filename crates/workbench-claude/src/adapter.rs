...
use crate::{
    CLAUDE_CODE_STREAM_PROTOCOL, ClaudeError, ClaudeErrorKind, ClaudeLaunchProfile, ShutdownReport,
    codec::encode_frame,
    process::{ClaudeProcess, preflight_subscription},
    protocol::{Inbound, interrupt_request, user_message},
};
...
/// Provider-port implementation backed by one fresh Claude Code child per attempt.
...
impl ClaudeProviderAdapter {
    /// Runs prompt-free authentication and stream initialization preflight.
    ///
    /// # Errors
    ///
    ...
    pub async fn connect(
            ...
            || expected_adapter_version.len() > MAX_ADAPTER_VERSION_BYTES
        {
            return Err(CoreError::new(
                ...
                "Claude Code adapter configuration is invalid",
            ));
        }
    ...
    fn require_dispatchable(&self) -> Result<(), ProviderFailure> {
        if self.shutting_down.load(Ordering::Acquire) {
            Err(definite_failure(
                ...
                "Claude Code provider is shutting down",
            ))
        } else {
            Ok(())
        }
...
impl ProviderAdapter for ClaudeProviderAdapter {
    ...
    async fn resume_session(
        ...
        Err(definite_failure(
            ...
            "Claude Code provider session resume is unavailable",
        ))
    }
    ...
    async fn prompt_stream(
        ...
        let message = user_message(&provider_session_id, prompt.content.as_str());
        encode_frame(&message).map_err(|error| setup_failure(&error))?;
        let mut process = ClaudeProcess::spawn_initialized(
            &self.profile,
            &self.adapter_version,
            Some(&prompt.runtime_model),
            &provider_session_id,
        )
        .await
        ...
        let write = process.write(&message);
        let write_result = tokio::time::timeout(self.cancellation_deadline, write).await;
        let write_failure = match write_result {
            Ok(Ok(())) => None,
            Ok(Err(_)) | Err(_) => Some(uncertain_failure()),
        };
...
async fn confirm_cancellation(
    ...
    let request_id = format!("interrupt-{}", Uuid::now_v7());
    let confirmation = async {
        process.write(&interrupt_request(&request_id)).await?;
        let mut acknowledged = false;
        loop {
            let Some(inbound) = process.next().await? else {
                return Ok::<bool, ClaudeError>(false);
            };
            match inbound {
                ...
                Inbound::Result {
                    ...
                    && matches!(reason.as_str(), "aborted_streaming" | "aborted_tools") =>
                {
                    return Ok(true);
                }
                Inbound::Result { .. } => return Ok(false),
                Inbound::SystemInit { .. }
                | Inbound::ControlResponse { .. }
                | Inbound::TextDelta(_)
                | Inbound::Assistant { .. }
...
fn normalize(
    ...
    match inbound {
        Inbound::TextDelta(text) => {
            if text.is_empty() {
                return Vec::new();
            }
            state.emitted_text_delta = true;
            vec![content_output("assistant_text_delta", text)]
        }
        Inbound::ToolStarted(name) => normalized_tool(name, state).into_iter().collect(),
        Inbound::Assistant { text, tools } => {
            let mut outputs = tools
                .into_iter()
                .filter_map(|name| normalized_tool(name, state))
                .collect::<Vec<_>>();
            if !state.emitted_text_delta {
                outputs.extend(
                    text.into_iter()
                        .filter(|value| !value.is_empty())
                        .map(|value| content_output("assistant_text", value)),
                );
            }
            outputs
        }
        Inbound::Result {
            ...
            if terminal_reason
                .as_deref()
                .is_some_and(|reason| matches!(reason, "aborted_streaming" | "aborted_tools"))
            {
                return vec![Err(uncertain_failure())];
            }
            if !is_error && subtype == "success" && terminal_reason.as_deref() == Some("completed")
            {
                vec![Ok(ProviderOutput::Completed {
                    summary: "Claude Code completed the prompt".to_owned(),
                })]
            } else if is_error
                && matches!(
                    subtype.as_str(),
                ...
                )
            {
                vec![Err(definite_failure(
                    FailureCategory::ProviderUnavailable,
                    "Claude Code reported a provider failure",
                ))]
            } else {
                vec![Err(uncertain_failure())]
            }
        }
        Inbound::SystemInit { .. } | Inbound::ControlResponse { .. } => {
            vec![Err(uncertain_failure())]
        }
        Inbound::Ignored => Vec::new(),
    }
}
...
fn validate_runtime_model(model: &str) -> Result<(), ProviderFailure> {
        ...
        || model.chars().any(char::is_control)
    {
        Err(definite_failure(
            FailureCategory::CapabilityUnavailable,
            "Claude Code runtime model is invalid",
        ))
    } else {
        Ok(())
    }
...
fn setup_failure(error: &ClaudeError) -> ProviderFailure {
    let (category, message) = mapped_failure(error.kind());
    definite_failure(category, message)
}

fn core_error(error: &ClaudeError) -> CoreError {
    let (category, message) = mapped_failure(error.kind());
    CoreError::new(category, message)
}

fn mapped_failure(kind: ClaudeErrorKind) -> (FailureCategory, &'static str) {
    match kind {
        ClaudeErrorKind::InvalidConfiguration => (
            FailureCategory::InvalidRequest,
            "Claude Code launch configuration is invalid",
        ),
        ClaudeErrorKind::IncompatibleProtocol | ClaudeErrorKind::CapabilityUnavailable => (
            FailureCategory::CapabilityUnavailable,
            "Claude Code provider is incompatible",
        ),
        ClaudeErrorKind::Timeout => (
            FailureCategory::ProviderTimeout,
            "Claude Code provider operation timed out",
        ),
...
fn uncertain_failure() -> ProviderFailure {
    ProviderFailure {
        ...
        user_safe_message: "Claude Code outcome could not be confirmed".to_owned(),
        definite: false,
    }
}
...
#[cfg(test)]
mod tests {
    use workbench_core::ports::ProviderOutput;

    use std::time::Duration;

    use super::{
        Inbound, MAX_CANCELLATION_REAP_RESERVE, NormalizationState,
        cancellation_confirmation_budget, normalize,
    };

    #[test]
    fn partial_text_suppresses_the_final_copy() {
        let mut state = NormalizationState::default();
        assert!(matches!(
            normalize(Inbound::TextDelta("hello".to_owned()), &mut state).as_slice(),
    ...
    #[test]
    fn successful_result_is_the_only_completion_shape() {
        let mut state = NormalizationState::default();
        assert!(matches!(
            normalize(
                Inbound::Result {
        ...
        let structured_error = normalize(
            Inbound::Result {
                is_error: true,
        ...
        assert!(matches!(
            structured_error.as_slice(),
            [Err(failure)] if failure.definite
        ));
        for terminal_reason in [None, Some("future_reason".to_owned())] {
            let incompatible_success = normalize(
                Inbound::Result {
                    is_error: false,
                    subtype: "success".to_owned(),
                    terminal_reason,
    ...
    #[test]
    fn cancellation_budget_reserves_time_to_reap_the_child() {
        assert_eq!(
            cancellation_confirmation_budget(Duration::from_millis(4_500)),
            Duration::from_millis(4_050)
        );
        assert_eq!(
            Duration::from_millis(4_050) + MAX_CANCELLATION_REAP_RESERVE,
            Duration::from_millis(4_500)
        );
        assert!(!cancellation_confirmation_budget(Duration::from_millis(120)).is_zero());
    }
}
