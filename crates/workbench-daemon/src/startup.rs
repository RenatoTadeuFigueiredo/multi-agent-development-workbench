...
use workbench_config::{
    ConfigError, ConfigLayer, ConfigurationSnapshot, ResolvedConfiguration, WorkbenchConfiguration,
    WorkbenchLock,
...
impl StartupConfiguration {
    ...
    pub fn load_with_configuration(
        ...
        let expected = Self::inspect(repository_root, explicit_configuration)?;
        let lock_path = repository_root.join(".workbench/workbench.lock");
        let bytes = read_owned_regular_file(&lock_path, false).map_err(|_| {
    ...
    pub fn inspect(
        ...
        let layers = SourcePaths::discover(repository_root)?.load_existing()?;
        let mut layers = layers;
        if let Some(path) = explicit_configuration {
            layers.push(load_explicit_layer(path)?);
        }
        let resolved = resolve_with_builtins(&layers)?;
        let snapshot =
